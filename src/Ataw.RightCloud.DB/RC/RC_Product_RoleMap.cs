...
{
    // bd_User
    public class RC_Product_RoleMap : EntityTypeConfiguration<RC_Product_Role>
    {
        public RC_Product_RoleMap(string schema = "dbo")
        {
            ToTable(schema + ".RC_Product_Role");
            HasKey(x => x.FID);
            
             Property(x => x.FID).HasColumnName("FID").IsRequired().HasMaxLength(50);

             Property(x => x.RCPR_ProductFID).HasColumnName("RCPR_ProductFID").IsOptional();

             Property(x => x.RCPR_RoleFID).HasColumnName("RCPR_RoleFID").IsOptional();
