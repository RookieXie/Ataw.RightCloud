...
{
    // bd_User
    public class RC_Menu_RoleMap : EntityTypeConfiguration<RC_Menu_Role>
    {
        public RC_Menu_RoleMap(string schema = "dbo")
        {
            ToTable(schema + ".RC_Menu_Role");
            HasKey(x => x.FID);

            Property(x => x.FID).HasColumnName("FID").IsRequired().HasMaxLength(50);

            Property(x => x.RCMR_MenuFID).HasColumnName("RCMR_MenuFID").IsOptional();

            Property(x => x.RCMR_RoleFID).HasColumnName("RCMR_RoleFID").IsOptional();

            Property(x => x.RCMR_FControlUnitID).HasColumnName("RCMR_FControlUnitID").IsOptional();

            Property(x => x.RCMR_Button).HasColumnName("RCMR_Button").IsOptional();
