...
using Ataw.FrameworkExtend.Core;
...
namespace Ataw.RightCloud.Web
{
    public class NewRightConfigController : AtawBaseController
    {
        ...
        /// </summary>
        /// <returns></returns>
         [Doc("ProductDataConfig.xml转化为json数据")]
        ...
        public string GetProductXmlPage()
        {
            var path = Path.Combine(AtawAppContext.Current.BinPath, AtawApplicationConfig.REAL_PATH, "Product");
            var filese = Directory.GetFiles(path);
            RCProductXmlData ProductDataConfig = new RCProductXmlData();
            List<RCProductXml> productXmlList = new List<RCProductXml>();
            foreach (var filePath in filese)
            {
                var productsDetail = new RCProductXml();
                XmlService servive = new XmlService();
               var  productData = ProductData.ReadConfig(AtawAppContext.Current.BinPath, filePath);
                productsDetail.xmlPath = filePath.Substring(filePath.IndexOf("xml") + 4);
                var m = servive.getProductXml(productsDetail.xmlPath);
                productsDetail.xmlContent = m;
                productsDetail.xmlName = productData.Name;
                productXmlList.Add(productsDetail);
            }
            ProductDataConfig.productXmlList = productXmlList;
            return ReturnJson(ProductDataConfig);
        }
    }
} 